from Cope import todo, debug, untested, confidence, flattenList, depricated
...
def moveFocus(self, axis, dots):
    ...
    # amt = (S.settings.dotSpread / (self.width() / 2)) * dots
    amt = S.settings.dotSpread * dots
...
def updateMirror(self, setTo=None, setIndex=None, increment=None):
    ...
    # halfway += self.translation % S.settings.dotSpread
...
def updateSettings(self, settings):
    ...
    S.settings.dotSpread = settings.dotSpread.value()
    S.settings.dotSpreadMeasure = settings.dotSpreadMeasure.value()
    S.settings.dotSpreadUnit = settings.dotSpreadUnit.text()
...
@untested
def genDotArrayPoints(size:QSize, dotSpread, startPoint=Pair(0, 0), offScreenAmount=0):
